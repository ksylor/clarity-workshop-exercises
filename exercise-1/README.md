...
8. Now push up your new repo's contents by running `git push -u origin master` this will set the `master` branch in the remote repository named `origin` as the upstream tracking branch for your local master. We'll dig into what this means later in the day! You should see output similar to this:
